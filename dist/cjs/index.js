...
function brighten(color, lightenBy) {
    return tinycolor__default["default"](color).brighten(lightenBy).toString();
}
...
function checkReadable(color, level = 'AA') {
    ...
    if(tinycolor__default["default"].readability(color, '#000000') > ratio) {
...
function multiplyColor$1(color, multiplier) {
    let hsvColor = tinycolor__default["default"](color).toHsv();
    ...
    return tinycolor__default["default"](hsvColor).toHexString();
}

function relativeLuminance(color) {
    color = tinycolor__default["default"](color).toRgb();
...
function getReccommended(color, color2, ratio) {
    if(color2) {
        if(tinycolor__default["default"].readability(color, color2) < ration) {
            return recommendedColors(color, color2, ratio);
        }
        return tinycolor__default["default"](color).toHexString();
    }

    if(tinycolor__default["default"](color).isLight()) {
        if(tinycolor__default["default"].readability(color, '#ffffff') < ratio) {
...
function reccommendedColors(background, foreground, target) {
    let cr = tinycolor__default["default"].readability(background, foreground);

    let fore = tinycolor__default["default"](foreground).toRgbString();
    let back = tinycolor__default["default"](background).toRgbString();

    Math.abs(relativeLuminance(fore) - 0.5);
    Math.abs(relativeLuminance(back) - 0.5);

    let reccomended = tinycolor__default["default"](color).toHexString();
    let change, newCol;
    ...
    if(cr < target) {
        ...
        let hsv = tinycolor__default["default"](newCol).toHsv();
        hsv.s * hsv.v;
        let newFore = fore;
        let newBack = back;
        let oldFore, oldBack;
        ...
        while(cr < target) {
            if((newCol === '#ffffff') || (newCol === '#000000')) {
                if(changed = true) {
                    ...
                    hsv = tinycolor__default["default"](newCol).toHsv();
                    hsv.s * hsv*v;
                    changed = true;
                }
            }
            ...
            newCol = tinycolor__default["default"](hsv).toRgb();
            newCol = multiplyColor(newcol, Math.pow(mulitiplier, i));

            change === 'fore' ? newFore = newCol : newBack = newCol;

            cr = tinycolor__default["default"].readability(newBack, newFore);

        }

        reccomended = tinycolor__default["default"](newBack).toHexString();
    }
...
function generateColor(color, level = 'AA') {
    if(level === undefined) return tinycolor__default["default"](color).toHexString();
...
function generatePalette(color, paletteLength = 9, level = 'AA') {
    const hsl = tinycolor__default["default"](color).toHsl();
    const { h, s , l} = hsl;
